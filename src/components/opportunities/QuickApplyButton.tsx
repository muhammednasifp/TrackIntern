...
const colors = ['#7c3aed', '#ec4899', '#f59e0b', '#10b981', '#38bdf8', '#f97316'];
  ...
  for (let i = 0; i < 18; i += 1) {
    ...
    const animation = piece.animate(
      [
        ...
        {
          transform: `translate3d(${translateX / 2}px, 40vh, 0) rotate(${rotate / 2}deg)`,
          opacity: 1,
        },
        {
...
export const QuickApplyButton: React.FC<QuickApplyButtonProps> = ({
  ...
  const handleQuickApply = async () => {
    ...
    if (!studentId) {
      toast.error('Your student profile was not found. Please refresh and try again.');
      return;
    }
    ...
    if (!window.confirm('Apply with your current profile? You can add a cover letter later.')) {
      return;
    }
  ...
  const { label, icon } = useMemo(() => {
    ...
    if (status === 'loading') {
      return {
        label: 'Applying...',
        icon: <ArrowPathIcon className="h-5 w-5 animate-spin" aria-hidden="true" />,
      };
    }
  ...
  const tapAnimation = !isButtonDisabled && status !== 'loading' ? { scale: 0.97 } : undefined;
